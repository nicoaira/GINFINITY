...
Inference uses full `float32` model precision by default. For half `float16`
inference, load the model with `Ginfinity.load(full_precision=False)` or pass
`--half-precision` to an embedding CLI command.
