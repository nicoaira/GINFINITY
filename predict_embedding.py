...
def get_gin_embedding(
    ...
    """
    If L is provided, we generate embeddings for each window (subgraph slice)
    of length L.  We now apply a per-window 'mask_threshold' on the fraction
    of paired bases, skipping any window whose fraction < mask_threshold.
    """
    ...
    with torch.no_grad():
        if L is not None:
            ...
            # if the structure is too short, bail early
            if len(sorted_nodes) < L:
                return [(-1, "")]
            embeddings = []
            slices = generate_slices(graph, L, keep_paired_neighbors)
            for start_idx, subgraph_H in slices:
                # --- window-level complexity mask ---
                if mask_threshold > 0:
                    window_seq = structure[start_idx : start_idx + L]
                    if not window_seq:
                        continue
                    paired = sum(1 for c in window_seq if c != '.')
                    frac = paired / len(window_seq)
                    if frac < mask_threshold:
                        continue
                # ------------------------------------
            ...
            # no sub-windows: full-structure embedding
...
def generate_embedding_for_row(args):
    """
    args is a tuple:
      (idx, row, model, struct_col, device,
       graph_encoding, subgraphs, L,
       keep_paired_neighbors, mask_threshold)
    """
    idx, row, model, struct_col, device, enc, subgraphs, L, keep_pairs, mask_thresh = args
    structure = row[struct_col]
    validate_structure(structure)
    if subgraphs:
        lst = get_gin_embedding(
            model, enc, structure, device,
            L, keep_pairs, mask_threshold=mask_thresh
        )
        return [(idx, start, emb) for (start, emb) in lst]
    else:
        emb_list = get_gin_embedding(
            model, enc, structure, device,
            L=None, keep_paired_neighbors=keep_pairs, mask_threshold=0.0
        )
        return [(idx, None, emb_list[0][1])]
...
def generate_embeddings(
    ...
    with Pool(num_workers) as pool:
        try:
            for res in tqdm(
                pool.imap_unordered(generate_embedding_for_row, args_list),
                total=len(input_df),
                desc="Embedding"
            ):
                results.extend(res)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mask-threshold',         type=float, default=0.3,
                        help="Minimum fraction of paired bases per window; windows below are skipped (default 0.3)")
    ...
    # determine which column holds the structure
    ...
    # make output directory & set up logging
